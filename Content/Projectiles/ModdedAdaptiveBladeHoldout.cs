...
public class ModdedAdaptiveBladeHoldout : ModProjectile
{
	...
	public override void AI()
	{
		...
		Projectile.Center = Owner.MountedCenter + pointingRotation.ToRotationVector2() * 32f;
		...
		if (Owner.channel) //Keep the player's hands full relative to attack speed
		{
			...
			/*if (Owner.itemTime < 2) These should never be true because we're adding 2
			{
				Owner.itemTime = 2;
			}*/
	...
	public void ReleaseSword()
	{
		while (true)
		{
			...
			var damageBoost = Charge >= 60f ? 4f : 1 + Charge / 30f;
