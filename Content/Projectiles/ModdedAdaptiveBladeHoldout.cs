...
{
	public class ModdedAdaptiveBladeHoldout : ModProjectile, ILocalizedModType
	{
		...
		public void ReleaseSword()
		{
			if ((Charge <= 60f || (Owner.channel && autoRelease)) && trmodPlayer.swingAnimCompletion == 0)
				trmodPlayer.swingAnimCompletion += 0.00001f; // start the swing anim
			...
			for (int i = 0; i < shotsToFire; i++)
			{
				...
				int spawnedProjectile = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Owner.MountedCenter + (direction * 32f), Vector2.Zero, ModContent.ProjectileType<AdaptiveBladeSlash>(), damage, 1f, Owner.whoAmI);
